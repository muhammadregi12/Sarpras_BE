  ...
  try {
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        page,
        limit,
        totalPages: Math.ceil(supplier.count / limit),
      },
    });
  } catch (error) {
...
exports.getSupplierById = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Get Supplier By Id", data: supplier });
  } catch (error) {
    return res.status(500).json({
      message: "Internal Server Error",
      error: error.message, // FIX: sertakan error.message
    });
  }
};
...
exports.createSupplier = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "Create Supplier", data: supplier });
  } catch (error) {
    return res.status(500).json({
      message: "Internal Server Error",
      error: error.message,
...
exports.updateSupplier = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Update Supplier", data: updatedSupplier });
  } catch (error) {
    return res.status(500).json({
      message: "Internal Server Error",
      error: error.message, // FIX: sertakan error.message
...
exports.deleteSupplier = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Delete Supplier" });
  } catch (error) {
    return res.status(500).json({
      message: "Internal Server Error",
      error: error.message, // FIX: sertakan error.message
...
exports.exportPDFSupplier = async (req, res) => {
  try {
    ...
    const pdfBuffer = await generatePDF("supplier.html", {
...
exports.exportExcelSupplier = async (req, res) => {
  try {
    // FIX: tambah limit untuk cegah OOM
    const suppliers = await Supplier.findAll({
      order: [["name_supplier", "ASC"]],
      limit: EXPORT_LIMIT,
    });
